...
Vec3 se_camera3d_get_front(const SE_Camera3D *cam) {
    ...
    f32 pitch = cam->pitch;
    Vec3 camera_front = {0};
    camera_front.x = se_math_cos(yaw * SEMATH_DEG2RAD_MULTIPLIER) * se_math_cos(pitch * SEMATH_DEG2RAD_MULTIPLIER);
    camera_front.y = se_math_sin(pitch * SEMATH_DEG2RAD_MULTIPLIER);
    camera_front.z = se_math_sin(yaw * SEMATH_DEG2RAD_MULTIPLIER) * se_math_cos(pitch * SEMATH_DEG2RAD_MULTIPLIER);
    vec3_normalise(&camera_front);
    return camera_front;
}

Mat4 se_camera3d_get_view(const SE_Camera3D *cam) {
    Vec3 camera_front = se_camera3d_get_front(cam);
...
void se_camera3d_update_projection(SE_Camera3D *cam, i32 window_w, i32 window_h) {
    cam->view = se_camera3d_get_view(cam);
...
void se_camera3d_input(SE_Camera3D *camera, SE_Input *seinput) {
    { // movement
        ...
        i32 d       = se_input_is_key_down(seinput, SDL_SCANCODE_S) == true ? 1 : 0;
        i32 u       = se_input_is_key_down(seinput, SDL_SCANCODE_W) == true ? 1 : 0;
        i32 elevate = se_input_is_key_down(seinput, SDL_SCANCODE_E) == true ? 1 : 0;
        i32 dive    = se_input_is_key_down(seinput, SDL_SCANCODE_Q) == true ? 1 : 0;
    ...
    { // -- rotate camera
        ...
        if (mouse_state & SDL_BUTTON_RMASK) {
            ...
            se_ui_mouse_fps_deactivate(seinput);
        }
    }
}

void se_camera3d_get_raycast(SE_Camera3D *camera, SDL_Window *window, Vec3 *_raycast_dir, Vec3 *_raycast_origin) {
